...
if (isset($_POST['login'])) {
    ...
    if (!$result) {
        die("Shelter Query Error: " . mysqli_error($conn));
    }
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $_SESSION['name'] = $row['Shelter_Name'];
        $_SESSION['email'] = $row['Email'];
        $_SESSION['account_type'] = 'shelter';

        echo "<script>
                alert('Shelter Login Successful');
                window.location='shelterdashboard.php';
              </script>";
...
<body>
    ...
    <div id="right">
    <h1><img src="assets/ku.png" alt="logo" id="logo"></h1>
