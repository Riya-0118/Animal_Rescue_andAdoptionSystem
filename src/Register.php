<?php
    ...
    $checkShelter = mysqli_query(
        $conn,
        "SELECT * FROM Shelter WHERE Email='$email'"
    );
    if (mysqli_num_rows($checkUser) > 0 ||
        mysqli_num_rows($checkShelter) > 0) {

        echo "<script>alert('Email already exists');</script>";
    }else{
        if ($role == "Adopter/Rescuer") {
        ...
        elseif ($role == "Shelter") {
            ...
            (Shelter_Name, Email, Phone, Password)
            VALUES
            ('$name', '$email', '$phone', '$password')";
        ...
        if (mysqli_query($conn, $sql)) {
            ...
            echo "<script>
                alert('Shelter Registration Failed: " . mysqli_error($conn) . "');
              </script>";
        }
        }
    }
